...
INFO_MESSAGE_TEXT
  ...
  def nw_dice(base, modify, criticalText, fumbleText)
    ...
    setValue(:criticalValues, getValuesFromText(criticalText, [10]))
    setValue(:fumbleValues, getValuesFromText(fumbleText, [5]))
    total = 0
    output = ""

    debug('@criticalValues', getValue(:criticalValues))
    debug('@fumbleValues', getValue(:fumbleValues))

    dice_n, dice_str, = roll(2, 6, 0)

    total = 0

    if( getValue(:fumbleValues).include?(dice_n) )
  ...
  def setCriticalValues(text)
    setValue(:criticalValues, getValuesFromText(text, [10]))
  end
  ...
  def getCriticalValue(dice_n)
    return getValue(:criticalValues).include?(dice_n)
  end
end
