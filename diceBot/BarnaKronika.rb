...
INFO_MESSAGE_TEXT
  ...
  def check_barna_kronika(string)
    ...
    setValue(:isBattleMode, false)       # 0=判定モード, 1=戦闘モード
    criticalCallDice = 0         # 0=通常, 1〜6=クリティカルコール

    if( option )
      battleModeText, criticalCallDice = option.split(",").collect{|i|i.to_i}
      setValue(:isBattleMode, (battleModeText == 1))
    end

    debug("@isBattleMode", getValue(:isBattleMode))

    dice_str, suc, set, at_str = roll_barna_kronika(dice_n, criticalCallDice)

    output = "#{getValue(:nick_e)}: (#{string}) ＞ [#{dice_str}] ＞ "

    if( getValue(:isBattleMode) )
      output += at_str
    else
      debug("suc", suc)
      if(suc > 1)
  ...
  def roll_barna_kronika(dice_n, criticalCallDice)
    ...
    if( getValue(:isBattleMode) and suc < 2)
      at_str = "失敗"
    end
  ...
  def isCriticalCall(index, criticalCallDice)
    return false unless( getValue(:isBattleMode) )
